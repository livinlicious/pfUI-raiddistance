pfUI:RegisterModule("RaidDistance", "vanilla:tbc", function ()
  ...
  local function HandleSlashCommand(input)
    ...
    if not args[1] or args[1] == "help" then
      DEFAULT_CHAT_FRAME:AddMessage("RaidDistance Configuration:")
      DEFAULT_CHAT_FRAME:AddMessage("  Status: " .. (tonumber(C.RaidDistance.enabled) == 1 and "Enabled" or "Disabled"))
      DEFAULT_CHAT_FRAME:AddMessage("  Alignment: " .. (C.RaidDistance.alignment or "CENTER"))
      DEFAULT_CHAT_FRAME:AddMessage("Commands:")
      DEFAULT_CHAT_FRAME:AddMessage("  /raiddistance right - Right align")
      DEFAULT_CHAT_FRAME:AddMessage("  /raiddistance left - Left align") 
      DEFAULT_CHAT_FRAME:AddMessage("  /raiddistance center - Center align")
      DEFAULT_CHAT_FRAME:AddMessage("  /raiddistance toggle - Enable/disable")
      return
    end

    if args[1] == "right" then
      C.RaidDistance.alignment = "RIGHT"
      DEFAULT_CHAT_FRAME:AddMessage("RaidDistance: Right aligned")
      UpdateAllRaidFrames()
      return
    end

    if args[1] == "left" then
      C.RaidDistance.alignment = "LEFT"
      DEFAULT_CHAT_FRAME:AddMessage("RaidDistance: Left aligned")
      UpdateAllRaidFrames()
      return
    end

    if args[1] == "center" then
      C.RaidDistance.alignment = "CENTER"
      DEFAULT_CHAT_FRAME:AddMessage("RaidDistance: Center aligned")
      UpdateAllRaidFrames()
      return
    end

    if args[1] == "toggle" then
      local currentState = tonumber(C.RaidDistance.enabled) or 0
      C.RaidDistance.enabled = currentState == 1 and "0" or "1"
      DEFAULT_CHAT_FRAME:AddMessage("RaidDistance: " .. (tonumber(C.RaidDistance.enabled) == 1 and "Enabled" or "Disabled"))
      if tonumber(C.RaidDistance.enabled) == 0 then
        for frame, textFrame in pairs(distanceFrames) do
          textFrame:Hide()
        end
      end
      return
    ...
    DEFAULT_CHAT_FRAME:AddMessage("Unknown command. Type /raiddistance for help")
  end
  ...
  SLASH_RAIDDISTANCE1 = "/raiddistance"
  SLASH_RAIDDISTANCE2 = "/rdd"
  SlashCmdList["RAIDDISTANCE"] = HandleSlashCommand

  -- Initial update
  UpdateAllRaidFrames()

  DEFAULT_CHAT_FRAME:AddMessage("RaidDistance loaded - Type /raiddistance for options")
end)